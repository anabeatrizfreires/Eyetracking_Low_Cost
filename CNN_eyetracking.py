...
landmarks_predictor = dlib.shape_predictor(os.path.join(dirname, 'new_predictor.dat'))
...
def main():
    ...
    while True:
        ...
        eyes = eye_detector(frame)
        for eye in eyes:
            landmarks = landmarks_predictor(frame, eye)
...
def detect_landmarks(frame, scale_x=0, scale_y=0):
...
def segment_eyes(frame, landmarks, ow=160, oh=96):
    ...
    # Segment eyes
    for corner1, corner2, is_left in [(22, 8, False)]:
        from functools import reduce
        #t_corner1 = np.array([landmarks[corner1].x, landmarks[corner1].y])
        #t_corner2 =  np.array([landmarks[corner2].x, landmarks[corner2].y])
        c1 = reduce(lambda prev, l: l if prev.x < l.x else prev, landmarks)
        c2 = reduce(lambda prev, l: l if prev.x > l.x else prev, landmarks)
        t_corner1 = np.array([c1.x, c1.y])
        t_corner2 = np.array([c2.x, c2.y])
        print(t_corner1)

        x1, y1 = t_corner1
        x2, y2 = t_corner2
        eye_width = 2 * np.linalg.norm(t_corner1 - t_corner2)
        if eye_width == 0.0:
            return eyes
